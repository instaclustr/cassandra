...
public class GuardrailValuesTable extends AbstractMutableVirtualTable
{
    ...
    public DataSet data()
    {
        ...
        for (Map.Entry<String, ValuesGuardrailsMapper> entry : Guardrails.getValueGuardrails().entrySet())
        {
            ...
            Set<String> warned = mapper.warnedValuesSupplier.get();
            Set<String> disallowed = mapper.disallowedValuesSupplier.get();
            Set<String> ignored = mapper.ignoredValuesSupplier.get();

            result.row(name)
                  .column(WARNED_COLUMN, warned.isEmpty() ? null : warned)
                  .column(DISALLOWED_COLUMN, disallowed.isEmpty() ? null : disallowed)
                  .column(IGNORED_COLUMN, ignored.isEmpty() ? null : ignored);
        }
