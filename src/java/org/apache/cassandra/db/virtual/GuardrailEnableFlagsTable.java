...
public class GuardrailEnableFlagsTable extends AbstractMutableVirtualTable
{
    ...
    public DataSet data()
    {
        ...
        for (Map.Entry<String, Pair<Consumer<Boolean>, Supplier<Boolean>>> entry : Guardrails.getFlagGuardrails().entrySet())
        {
            String guardrailName = entry.getKey();
            Pair<Consumer<Boolean>, Supplier<Boolean>> setterAndGetter = entry.getValue();
            if (setterAndGetter == null)
                continue;

            Supplier<Boolean> getter = setterAndGetter.right;
            if (getter == null)
                continue;

            result.row(guardrailName).column(VALUE_COLUMN, getter.get());
        }
