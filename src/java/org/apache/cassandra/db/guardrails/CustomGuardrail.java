...
public class CustomGuardrail<VALUE> extends Guardrail
{
    ...
    private ValueValidator<VALUE> getValidator()
    {
        maybeInitialize();
        return holder.validator;
    }

    private ValueGenerator<VALUE> getGenerator()
    {
        maybeInitialize();
    ...
    public void guard(VALUE value, ClientState state)
    {
        ...
        if (validationResult.isPresent())
            ...
            currentValidator.shouldWarn(value).ifPresent(result -> warn(result.message, result.redactedMessage));
    }
    ...
    {
        return getGenerator().generate(size);
    }
