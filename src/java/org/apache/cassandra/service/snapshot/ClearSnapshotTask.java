...
public class ClearSnapshotTask implements Runnable
{
    ...
    private final Supplier<Iterable<TableSnapshot>> snapshotsSupplier;
    private final boolean deleteData;

    public ClearSnapshotTask(Predicate<TableSnapshot> predicateForToBeCleanedSnapshots,
                             Supplier<Iterable<TableSnapshot>> snapshotsSupplier,
                             boolean deleteData)
    {
    ...
    public void run()
    {
        for (TableSnapshot snapshot : snapshotsSupplier.get())
        {
            if (predicateForToBeCleanedSnapshots.test(snapshot))
            {
                ...
                //  TODO here we need to remove from iterator
                //  TODO how to remove when we have just iterator
            }
        }
    }
     ...
     */
    public static Predicate<TableSnapshot> getPredicateForCleanedSnapshots(String tag, Map<String, Object> options, String... keyspaceNames)
    {
        if (options == null)
        ...
        return shouldClearSnapshot(tag, Set.of(keyspaceNames), maxCreatedAt, false);
    }
