...
public class FormatFcts
{
     ...
     * looked at as the value is of unit 'ms' and it will be converted to a value of a unit which is closest to it. E.g.
    ...
    public static class FormatTimeFct extends NativeScalarFunction
    {
        ...
        private Pair<Double, String> convertValue(long valueToConvert) {
            ...
            long[] conversionFactors = {86400000, 3600000, 60000, 1000}; // Milliseconds in a day, hour, minute, second

            for (int i = 0; i < conversionFactors.length; i++)
            {
                if (valueToConvert >= conversionFactors[i])
                {
                    double convertedValue = (double) valueToConvert / conversionFactors[i];
        ...
        private static double getNanosPerUnit(TimeUnit unit) {
            switch (unit) {
                case NANOSECONDS: return 1.0;
                case MICROSECONDS: return 1_000.0;
                case MILLISECONDS: return 1_000_000.0;
                case SECONDS: return 1_000_000_000.0;
                case MINUTES: return 60.0 * 1_000_000_000.0;
                case HOURS: return 3600.0 * 1_000_000_000.0;
                case DAYS: return 86400.0 * 1_000_000_000.0;
