...
public class GuardrailAllowUncompressedTablesTest extends GuardrailTester
{
    ...
    public void createSuccess()
    {
        ...
        schemaChange(String.format("CREATE TABLE %s.%s (k int primary key, v int) WITH compression={'class':''}", KEYSPACE, table));
    ...
    public void createFailure() throws Throwable
    {
        setGuardrail(false);
        String table = createTableName();
        assertFails(String.format("CREATE TABLE %s.%s (k int primary key, v int) WITH compression={'class':''}", KEYSPACE, table), "Uncompressed table is not allowed");
    }
    ...
    public void alterSuccess()
    {
        ...
        schemaChange(String.format("ALTER TABLE %s.%s WITH compression = {'class': ''}", KEYSPACE, table));
        TableMetadata tmd = Schema.instance.getTableMetadata(KEYSPACE, table);
        Assert.assertFalse(tmd.params.compression.isEnabled());
    }
    ...
    public void alterFailure() throws Throwable
    {
        ...
        String table = createTableName();
        schemaChange(String.format("CREATE TABLE %s.%s (k int primary key, v int)", KEYSPACE, table));
        assertFails(String.format("ALTER TABLE %s.%s WITH compression = {'class': ''}", KEYSPACE, table), "Uncompressed table is not allowed");
    }
}
