 ...
 * <pre>
 *   On fixed code, CBUtil.readValue rejects length &gt; cb.readableBytes() with a
 *   ProtocolException. The decoder catches it, wraps it in an ErrorMessage, and
 ...
 * <pre>
 *   CBUtil.readValue does new byte[Integer.MAX_VALUE] (via cb.nioBuffer) and the JVM
 *   throws OutOfMemoryError ("Requested array size exceeds VM limit"). In a production
 *   Cassandra process with -XX:OnOutOfMemoryError=kill -9 %p the JVM is killed
 *   immediately. In this test JVM that flag is not set, so the OOM is caught in the
 *   protocol decoder, wrapped as ErrorMessage, and returned to the client — but the
 *   response text references OutOfMemoryError, NOT the post-fix bound-check text, so
 *   the contains-assertion below fails.
 * </pre>
 *
 ...
 *   means the bug was not fixed at CBUtil.readValue.
 * </pre>
 */
public class CBUtilReadValueDoSNativeProtocolTest extends CQLTester
{
    @BeforeClass
    ...
    //          swapping AUTH_RESPONSE's codec via Message.Type#unsafeSetCodec.
