...
public class SnapshotTest extends TestBaseImpl
{
    ...
    public void testSimpleSnapshot() throws Throwable
    {
                                             ...
                                             .start()))
        {
            ...
            cluster.schemaChange(withKeyspace("create table %s.tblconstraints2 (id int primary key, x int check NOT_NULL(x), v text check LENGTH(v) > 10)"));
            cluster.schemaChange(withKeyspace("create table %s.tblconstraints3 (id int primary key, x int check STRICTLY_NOT_NULL(x), v text check LENGTH(v) > 10)"));
