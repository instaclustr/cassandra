...
public class GuardrailNonPartitionRestrictedQueryTest extends GuardrailTester
{
    ...
    {
        enableGuardrail();
        assertThat(executeSelectViaDriver()).isEmpty();

        disableGuardrail();

        try
        {
            executeSelectViaDriver();
        ...
        // even we disabled guardrail, if we query by primary key and clustering column, if passes
        executeViaDriver(String.format("SELECT * from %s.%s WHERE k = 0 AND c = 0", KEYSPACE, tableName));
        executeViaDriver(String.format("SELECT * from %s.%s WHERE k = 0", KEYSPACE, tableName));
     ...
     */
    private List<String> executeSelectViaDriver()
    {
        return executeViaDriver(String.format("SELECT * from %s.%s WHERE v1 = 0", KEYSPACE, tableName));
    }
    ...
    private void enableGuardrail()
    {
        ...
        cluster.forEach(instance -> assertTrue(instance.callsOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> Guardrails.instance.getNonPartitionRestrictedIndexQueryEnabled()).call()));
    }
    ...
    private void disableGuardrail()
    {
        ...
        cluster.forEach(instance -> assertFalse(instance.callsOnInstance((IIsolatedExecutor.SerializableCallable<Boolean>) () -> Guardrails.instance.getNonPartitionRestrictedIndexQueryEnabled()).call()));
    }
    ...
    {
        return cluster.stream().mapToLong(i -> i.metrics().getCounter("org.apache.cassandra.metrics.keyspace.TooManyIndexReadSSTablesWarnings.distributed_test_keyspace")).sum();
    }

    private long totalAborts()
    {
        return cluster.stream().mapToLong(i -> i.metrics().getCounter("org.apache.cassandra.metrics.keyspace.TooManyIndexReadSSTablesAborts.distributed_test_keyspace")).sum();
    }
}
