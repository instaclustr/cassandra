...
public class CompactionHistorySystemTableUpgradeTest extends UpgradeTestBase
{
    ...
    public void compactionHistorySystemTableTest() throws Throwable
    {
        ...
        .setup((cluster) -> {
            //create table
            cluster.schemaChange("CREATE TABLE " + KEYSPACE + ".tb (pk text PRIMARY KEY, c1 text, c2 int, c3 int)");
        ...
        .runAfterClusterUpgrade((cluster) -> {
            ...
            ToolResult toolHistory = invokeNodetoolJvmDtest(cluster.get(1), "compactionhistory");
            toolHistory.assertOnCleanExit();
            String stdout = toolHistory.getStdout();
            String[] resultArray = stdout.split(System.lineSeparator());
            assertTrue(stream(resultArray).anyMatch(result -> result.contains(OperationType.UNKNOWN.type)
